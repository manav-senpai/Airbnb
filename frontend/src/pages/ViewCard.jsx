...
import { FaStar, FaMapMarkerAlt, FaRegHeart, FaShare, FaPlus, FaMinus } from "react-icons/fa";
...
function ViewCard() {
    ...
    const [guestCount, setGuestCount] = useState(cardDetails?.guestCapacity || 1);
    const [bedroomCount, setBedroomCount] = useState(cardDetails?.bedroomCount || 1);
    const [bathroomCount, setBathroomCount] = useState(cardDetails?.bathroomCount || 1);
    ...
    // Helper for Counter UI in Modal
    const EditCounter = ({ label, value, setValue }) => (
        <div className="flex items-center justify-between border-b py-2">
            <span className="text-gray-700 font-medium">{label}</span>
            <div className="flex items-center gap-3">
                <button 
                    onClick={() => setValue(Math.max(1, value - 1))}
                    className="p-1 rounded-full border hover:border-black"
                >
                    <FaMinus size={10} />
                </button>
                ...
                <div className='mb-6'>
                    ...
                    <div className='flex items-center justify-between text-sm text-gray-600 underline'>
                        <div className='flex items-center gap-2'>
                            ...
                            {/* FIX: Removed hardcoded 4.8 */}
                            <span className='font-semibold'>{cardDetails.averageRating ? cardDetails.averageRating : "New"}</span>
                            <span>•</span>
                            <span className='font-semibold capitalize'>{cardDetails.city}, {cardDetails.landmark || cardDetails.landMark}</span>
                        </div>
                        <div className='flex gap-4'>
                            {/* FIX: Added Share Handler */}
                            <button onClick={handleShare} className='flex items-center gap-1 hover:bg-gray-100 px-2 py-1 rounded'><FaShare /> Share</button>
                            {/* FIX: Commented out Save button */}
                        ...
                        <div className='sticky top-28 border border-gray-200 shadow-xl rounded-2xl p-6'>
                            <div className='flex justify-between items-baseline mb-6'>
                                ...
                                <div className='flex items-center gap-1 text-sm font-semibold'>
                                    <FaStar className='text-rose-500' /> {cardDetails.averageRating ? cardDetails.averageRating : "New"}
                                </div>
                            </div>
                ...
                <div className='fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm'>
                    <div className='bg-white w-full max-w-lg rounded-2xl p-6 relative max-h-[90vh] overflow-y-auto shadow-2xl'>
                        ...
                        <div className='space-y-4'>
                            ...
                            {/* FIX: Added Image Inputs */}
                            <div className="grid grid-cols-1 gap-2 border p-3 rounded-lg bg-gray-50">
                                <span className="font-bold text-sm">Update Images</span>
                                <div className="text-xs">Image 1 (Main)</div>
                                <input type="file" onChange={(e) => setBackEndImage1(e.target.files[0])} className="text-sm" />
                                <div className="text-xs">Image 2</div>
                                <input type="file" onChange={(e) => setBackEndImage2(e.target.files[0])} className="text-sm" />
                                <div className="text-xs">Image 3</div>
                                <input type="file" onChange={(e) => setBackEndImage3(e.target.files[0])} className="text-sm" />
                            </div>

                            {/* FIX: Added Counters */}
                            <div className="flex flex-col gap-2 p-3 border rounded-lg">
                                <EditCounter label="Guests" value={guestCount} setValue={setGuestCount} />
                                <EditCounter label="Bedrooms" value={bedroomCount} setValue={setBedroomCount} />
                                <EditCounter label="Bathrooms" value={bathroomCount} setValue={setBathroomCount} />
                            </div>
